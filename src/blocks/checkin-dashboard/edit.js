...
export default function Edit( { attributes, setAttributes } ) {
	...
	/**
	 * Render the main content area based on loading/data state.
	 *
	 * @return {JSX.Element} Content element.
	 */
	const renderContent = () => {
		if ( loading ) {
			return (
	...
	useEffect( () => {
		...
		Promise.all( [
			apiFetch( {
				path: `/post-kinds-indieweb/v1/checkins?per_page=${ limit }`,
			} ),
